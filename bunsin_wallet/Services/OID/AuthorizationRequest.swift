...
func decodeUriAsJson(uri: String) throws -> [String: Any] {
    ...
    guard let params = URLComponents(string: uri)?.queryItems else {
        throw IllegalArgumentException.badParams
    }
...
func processPresentationDefinition(
    ...
    if let ro = requestObject {
        print("processing target : \(requestObject)")
        if let presentationDefinition = ro.presentationDefinition {
            ...
            if let uri = presentationDefinitionUri, let requestUri = URL(string: uri) {
                do {
                    ...
                    print("unable to fetch and decode presentationDefinition from uri : \(error)")
                    throw AuthorizationError.invalidClientMetadata
                }
            }
            else {
                print("invalid presentationDefinitionUri : \(presentationDefinitionUri)")
                return nil
            }
        }
    }
    else {
        print("processing target : \(authorizationRequest)")
