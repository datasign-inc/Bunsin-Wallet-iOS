...
extension Datastore_CredentialData {
    ...
    private func parsedMetaData() -> CredentialIssuerMetadata? {
    ...
    func getCredentialName(locale: String = "ja-JP") -> String? {
        guard let metaData = self.parsedMetaData(),
            let supported = metaData.credentialConfigurationsSupported[self.type],
            let displays = supported.display,
            let firstDisplay = displays.first  // todo: localeに適したものを配列から取得する
        else {
            return nil
        }
    ...
    func toCredential() -> Credential? {
        ...
        let result = Credential(
            ...
            issuedAt: iat,
            backgroundImageUrl: self.getBackgroundImage(),
