...
struct Disclosure: Codable {
    ...
    init(disclosure: String?, key: String?, value: String?, localizedKey: String? = nil) {
    ...
    func translateKey(locale: String, credentialConfiguration: CredentialConfiguration) -> String? {
        if let already = self.localizedKey {
            return already
        }
        if let key = self.key {
            return credentialConfiguration.getLocalizedClaimName(
                locale: locale, targetClaimName: key)
        }
        return self.key
    }
}
