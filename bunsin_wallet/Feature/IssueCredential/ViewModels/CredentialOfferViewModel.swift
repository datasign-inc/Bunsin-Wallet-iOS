...
enum CredentialOfferViewModelError: Error {
    ...
    case DataRequiredForIssuanceCouldNotBeLoad
}

@Observable class CredentialOfferViewModel {
    ...
    func validateRequiredData() throws {
        guard let issuerMetaData = dataModel.metaData?.credentialIssuerMetadata,
            let targetCredentialId = dataModel.targetCredentialId,
            let targetCredential = issuerMetaData.credentialConfigurationsSupported[
                targetCredentialId]
        else {
            throw CredentialOfferViewModelError.DataRequiredForIssuanceCouldNotBeLoad
        }
    }
