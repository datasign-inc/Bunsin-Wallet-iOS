...
final class localizedClaimNamesTests: XCTestCase {
    func testGetLocalizedClaimNames() {
        ...
        let localizedNamesEN = claims.getLocalizedClaimNames(locale: "en-US")
        XCTAssertEqual(localizedNamesEN, ["Given Name"])

        let localizedNamesJP = claims.getLocalizedClaimNames(locale: "ja_JP")
        XCTAssertEqual(localizedNamesJP, ["名"])

        let localizedNamesDefault = claims.getLocalizedClaimNames(locale: "fr-FR")
...
final class DecodingVCIMetadataTests: XCTestCase {
    ...
    func testDecodeVcSdJwtMetadata() throws {
        ...
        guard let claimNames = credentialConfig?.getLocalizedClaimNames(locale: "ja-JP") else {
            XCTFail("claimNames must be array")
            return
        }

        XCTAssertTrue(!(claimNames.count == 0))
